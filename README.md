...
一个用于教学和技术分享的 PHP 8.2 Agent 项目：不依赖 Agent 框架，从最小 LLM 调用开始，逐步实现 Function Calling、ReAct、Plan-and-Execute、stdio/HTTP MCP、Skills、Memory、上下文压缩、Sub-agent 和交互式 Coding Agent。
...
- 实现 stdio 与 Streamable HTTP MCP Tools、Skills 渐进式加载和文件型长期 Memory；
- 实现隔离消息历史、受限工具集和防递归的同步 Sub-agent；
...
`delegate_task` 可把独立任务交给 Sub-agent：`research` 模式只读，`workspace` 模式允许文件写入和精确编辑，但两种模式都不能执行 Shell 或递归委派。

### 4. 可选 Salora Streamable HTTP MCP

启动本地 Salora MCP Server：
...
JWT 只应保存在被 Git 忽略的 `.env` 中，不要写进 Tool 参数、Prompt、日志、README 或提交历史。MCP Server 只允许调用 Catalog 中发布的只读 Tool，并继续由 CRM 执行最终权限判断。
...
- `bin/05_modern_agent.php`：stdio/HTTP MCP、Skills、Memory、Context Manager 和 Sub-agent；
...
测试覆盖工具注册与执行、Sub-agent 权限收窄、目录边界、Memory、Skills、MCP Schema、HTTP transport 校验、上下文压缩、UTF-8 终端输入、日志时区和命令行参数解析。
...
这是教学实现，不是生产级 Agent 平台。当前 Sub-agent 为同步执行；尚未实现流式输出、向量数据库、完整 RAG、并行 Sub-agent、容器沙箱、多用户隔离或完整 MCP 协议能力。
